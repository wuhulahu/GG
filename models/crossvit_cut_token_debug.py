...
from models.modules import custom_activation
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=(224, 224), patch_size=(8, 16), in_chans=3, num_classes=1000, embed_dim=(192, 384),
                 ...
                 drop_path_rate=0., code_length=16, synchronized=False, weight_remain=0.75, soft_per_example=True, cut=False, sigma=0.5, limit=0.5,
                 hybrid_backbone=None, norm_layer=nn.LayerNorm, multi_conv=False,
                 use_custom_activation=False, ratio_att=True, **kwargs):
        super().__init__()
    ...
    def generate_row_column(self, bs, num_remain_tokens, soft_per_example, num_samples=None):
        if soft_per_example:
            ...
            for i in range(bs):
                ...
                sampled_indices_idx_range = torch.randperm(num_remain_tokens)[:this_num_samples]
            ...
            sampled_indices_idx_range = torch.randperm(num_remain_tokens)[:num_samples]
            # 生成row和colum，进行采样, 修改tensor元素时， a[row, column]是可以的，a[idx]不行，这里的idx是指索引矩阵
    ...
    def forward_features(self, x, mask):
        ...
        mask_one = [torch.ones((B, self.pos_embed[0].shape[1]), requires_grad=False).to(x.device),
                torch.ones((B, self.pos_embed[1].shape[1]), requires_grad=False).to(x.device)]

        for layer, blk in enumerate(self.blocks):
            if self.layer_cut == layer:
                # self.layer_cut：-1,就是不cut；0就是从头cut
                mask_one = mask
            xs = blk(xs, mask_one)
    ...
    def generate_mask(self, x):
        with torch.no_grad():
            ...
            # generate_mask,两个通道有两个mask
            mask = [torch.ones((B, self.pos_embed[0].shape[1]), requires_grad=False).to(x.device),
                    torch.ones((B, self.pos_embed[1].shape[1]), requires_grad=False).to(x.device)]
            label_soft_weight = []
            ...
            if self.training:
                self.eval()
                xs = self.features_map_encoder(x)
                ...
                for i, att_scores in enumerate(att_score_list):
                    # 将att_scores按行排序
                    _, idx = torch.sort(att_scores, dim=-1, descending=True)

                    # 计算前percentile分位数所在的位置
                    k = int(idx.size(-1) * self.weight_remain)

                    # 将样本数量限制在最小值和最大值之间
                    idx_range = idx[:, :k]

                    row, column, label_adjust = self.sample_mask_token_debug(idx_range, row_list[i], column_list[i], sampe_ratio[i])

                    if self.ratio_att:
                        label_adjust = [1 - att_scores[n][per_column].sum() for n, per_column in enumerate(column)]
                    if self.use_custom_activation:
                        label_adjust = [custom_activation(torch.Tensor(per_label_adjust)) for per_label_adjust in label_adjust]
        ...
        return mask, torch.tensor(label_soft_weight).mean(dim=0)

    def forward(self, x, targets=None, phase='train', similarity=True):

        mask, label_soft_weight = self.generate_mask(x)
        xs = self.forward_features(x, mask)   # xs是各尺寸的cls_token
...
@register_model
def crossvit_small_224_cut_debug(pretrained=False, erasing_model_path=None,**kwargs):
