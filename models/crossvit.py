...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=(224, 224), patch_size=(8, 16), in_chans=3, num_classes=1000, embed_dim=(192, 384), depth=([1, 3, 1], [1, 3, 1], [1, 3, 1]),
                 ...
                 drop_path_rate=0., code_length=16,  hybrid_backbone=None, norm_layer=nn.LayerNorm, multi_conv=False, **kwargs):
        super().__init__()
        ...
        # note hash_layer 此处的num_c，lasses没有额外含义，只是cls_token做了投影之后的纬度而已，还没有改参数名
    ...
    def forward_features(self, x):
        ...
        # note out是取出了xs里的cls_token
        out = [x[:, 0] for x in xs]

        return out

    def forward(self, x):
        # note 取出各尺寸的cls_token
