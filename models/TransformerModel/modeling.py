...
import models.TransformerModel.vit_configs as configs


from .modeling_resnet import ResNetV2
from models.modules import generate_att_mask_convs
...
class VisionTransformer(nn.Module):
    def __init__(self, config, code_length, weight_remain, sigma, limit, cut=False, vis=False, use_custom_activation=False, ratio_att=True):
    ...
    def _init_weights(self):
        ...
        self.load_from(np.load('checkpoints/ViT-B_32.npz'))
    ...
    def get_att_mask(self, x):
        ...
        #   torch.matmul(image_block_representations, cls_representation.transpose(1, 2))
...
VIT_CONFIGS = {
    'ViT-B_16s': configs.get_b16s_config(),
...
def VisionTransformer_load(arch, code_length, weight_remain, sigma, limit, cut=False, use_custom_activation=False, ratio_att=True, erasing_model_path=None):
    if 'cut' in arch:
        cut = True
    model = VisionTransformer(config=VIT_CONFIGS["ViT-B_32"], code_length=code_length, weight_remain=weight_remain, sigma=sigma, limit=limit, cut=cut,
                              use_custom_activation=use_custom_activation, ratio_att=ratio_att)


    if erasing_model_path is not None:
        model.load_state_dict(torch.load(erasing_model_path)['model'])
    return model