...
def train(
    ...
    for it in range(max_iter):
        ...
        for data, targets, index in train_dataloader:
            ...
            if args.soft and args.cut:
                print("soft_data and soft_conv cannot be used simultaneously.")
                exit()
            targets = targets[0]

            U_batch, dsdh_cut_token = model(data)
            U_batch = U_batch.t()
            U[:, index] = U_batch.data

            dsdh_cut_token = dsdh_cut_token.to(U_batch.device)
            if args.no_q:
                # flag args.no_q 是消融
                dsdh_cut_token = None
            elif dsdh_cut_token.shape[0] != data.shape[0]:
                dsdh_cut_token = dsdh_cut_token.repeat(data.shape[0])
                # soft_weight = None
            loss = criterion(U_batch, U, S[:, index], B[:, index], W, targets, soft_weight=dsdh_cut_token)
            # loss = criterion(U_batch, U, targets, B[:, index])
            if args.triplet and data.shape[0] > 20:
                # 一个batch太小的话，triplet_loss无法计算
                tri_loss = triplet_loss(U_batch.t(), S[:, index][index,:], soft_weight=dsdh_cut_token)
